...
ENTRYPOINT ["/bin/bash", "-c", "cat /etc/nginx/conf.d/00-log-format.template /etc/nginx/conf.d/00-default.template > /etc/nginx/conf.d/default.template && envsubst '${APPLICATION}' < /etc/nginx/conf.d/default.template > /etc/nginx/conf.d/00-default.conf && exec nginx -g 'daemon off;'"]
